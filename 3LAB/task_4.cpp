...
void findRightGK(string img_path, string ref_img_path) {
  ...
  findContours(src_thr_clone, contours, hierarchy, RETR_EXTERNAL,
               CHAIN_APPROX_SIMPLE);
  findContours(ref_thr_clone, ref_countour, hierarchy, RETR_EXTERNAL,
               CHAIN_APPROX_SIMPLE);
  vector<int> valid_id;
  ...
  for (int i = 0; i < contours.size(); i++) {
    double dif =
        matchShapes(contours.at(i), ref_countour.at(0), CONTOURS_MATCH_I2, 0);
